...
uint32_t task1Period;
uint32_t task2Period;
uint32_t mail;
uint32_t count;
int32_t send =0;
int32_t ack=0;
...
void (*PerodicTask01)(void);
void (*PerodicTask02)(void);
...
int OS_AddPeriodicEventThreads(void(*thread1)(void), uint32_t period1,
  ...
  //***YOU IMPLEMENT THIS FUNCTION*****
	...
	task1Period = period1;
	task2Period = period2;
	PerodicTask01 = thread1;
	PerodicTask02 = thread2;
	EndCritical(stat);
  return 1;
}
...
void Scheduler(void){ // every time slice
  ...
  //***YOU IMPLEMENT THIS FUNCTION*****
	count = (count+1)%(task1Period*task2Period); //count+1 % max value of the period
	if ((count%task1Period) ==0){
		(*PerodicTask01)();}
	if ((count%task2Period) ==1){
		(*PerodicTask02)();}
